...
std::shared_ptr<DamageModifier> DamageModifier::makeModifier(uint8_t stance, uint8_t modType, uint16_t amount, 
	ModFactor factor, uint8_t chance, CombatType_t combatType, CombatOrigin source, 
	CreatureType_t creatureType, RaceType_t race, std::string_view creatureName) {
...
void DamageModifier::increaseValue(uint16_t amount) {
	if ((m_value + amount) <= std::numeric_limits<uint16_t>::max()) {
		...
		std::cout << "[WARNING] Amount exceeded numeric limits for uint16_t. m_value set to limit." << "\n";
	}
}
