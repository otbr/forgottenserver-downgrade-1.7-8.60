...
local EventMeta = {
	__newindex = function(self, key, callback)
		...
		if not eventType then
			debugPrint(string.format("[Warning - Event::%s] is not a valid callback.",
			                         key))
			return
		end
		...
		if type(callback) ~= "function" then
			debugPrint(
				string.format("[Warning - Event::%s] a function is expected.", key))
			return
		end
...
local function register(self, triggerIndex)
	...
	if not eventType or not callback then
		debugPrint(
			"[Warning - Event::register] need to setup a callback before you can register.")
		return false
	end
	...
	table.sort(events,
	           function(ecl, ecr) return ecl.triggerIndex < ecr.triggerIndex end)
	self.eventType = nil
	self.callback = nil
	return true
end
...
Event = setmetatable({
	clear = function(self)
		EventData = {}
		for i = 1, autoID do EventData[i] = {maxn = 0} end
	end
}, {
	__call = function(self) return setmetatable({register = register}, EventMeta) end,
	...
	__index = function(self, key)
		...
		return function(...)
			...
			for index = 1, eventsCount do
				...
				for i, value in pairs(updateableParams) do args[i] = results[value] end
			end
		end
	end
})
...
hasEvent = setmetatable({}, {
	__index = function(self, key)
		local callback = callbacks[key]
		if callback then return EventData[callback].maxn > 0 end
	end
})
